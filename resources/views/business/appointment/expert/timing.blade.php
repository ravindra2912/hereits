...
<div class="container-fluid px-4">
    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-4 border-bottom">
        <div>
            <h1 class="h3 fw-bold mb-0">Expert Timing</h1>
            <p class="text-muted small mb-0">Manage weekly schedule for <span class="fw-bold text-primary">{{ $expert->expert_name }}</span></p>
        </div>
...
<div class="modal fade" id="modal-timing" tabindex="-1" aria-labelledby="modalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <form action="{{ route('business.appointment.expert.timing.store', $expert->id) }}" id="timing-form" data-action="reload" class="modal-content formaction border-0 shadow-lg rounded-4" method="POST">
            @csrf
            <input type="hidden" name="timing_id" id="timing_id">
            <div class="modal-header border-bottom-0 pt-4 px-4">
                <h5 class="modal-title fw-bold" id="modalLabel">Add Time Slot</h5>
            ...
            <div class="modal-body px-4 pb-4">
                ...
                <div class="mb-2" id="apply_all_wrapper">
                    <div class="form-check form-switch p-3 bg-light rounded-3 shadow-sm border">
                        ...
                        <small class="text-muted d-block mt-1 ps-5">This will add this time slot to every day of the week for this expert.</small>
                    </div>
                </div>
            </div>
...
<script>
    ...
    function editTiming(time) {
        ...
        $('#apply_all_wrapper').addClass('d-none');
        timingModal.show();
    }
    ...
    function destroy(id) {
        Swal.fire({
            ...
            text: "Are you sure you want to remove this timing for the expert?",
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: "{{ route('business.appointment.expert.timing.destroy') }}",
                    type: "POST",
                    data: { id: id },
