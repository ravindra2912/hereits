...
<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-4 border-bottom">
    ...
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb mb-0">
            ...
            <li class="breadcrumb-item"><a href="{{ route('business.service.plan') }}" class="text-decoration-none">Service Plans</a></li>
        ...
        <div class="card border-0 shadow-sm rounded-4 overflow-hidden">
            <div class="row g-0">
                ...
                <div class="col-md-5 bg-primary bg-gradient text-white p-5 d-flex flex-column justify-content-center text-center">
                    ...
                    $total_price = ($quantity * ($plan->per_service_price ?? 0)) * $plan->duration;
                    @endphp
                    <h1 class="display-3 fw-bold mb-3">
                        <span class="fs-2 align-top">₹</span>{{ number_format($total_price, 0) }}
                    </h1>
                    <p class="fs-5 opacity-75 mb-4">{{ $quantity }} Services for {{ $plan->duration }} {{ $plan->duration > 1 ? 'Months' : 'Month' }}</p>
...
<form id="purchaseForm" method="POST" action="{{ route('business.service.plan.buy') }}" class="d-none">
    @csrf
...
<script>
    $(document).ready(function() {
        ...
        $('#apply_coupon').click(function() {
            ...
            $.ajax({
                url: "{{ route('business.service.plan.validate_coupon') }}",
                type: "POST",
                data: {
                    _token: "{{ csrf_token() }}",
                    coupon_code: code,
        ...
        $('#pay_now_btn').click(function() {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    $.ajax({
                        url: "{{ route('business.service.plan.buy') }}",
                        type: "POST",
                        data: formData,
                        success: function(response) {
                            if (response.success) {
