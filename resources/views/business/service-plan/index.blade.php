...
@section('title', 'Service Listing Plans')

@section('content')

<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-4 border-bottom">
    <h1 class="h2">Service Listing Plans</h1>
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb mb-0">
            <li class="breadcrumb-item"><a href="{{ route('business.dashboard') }}" class="text-decoration-none">Home</a></li>
            <li class="breadcrumb-item"><a href="{{ route('business.service.index') }}" class="text-decoration-none">Services</a></li>
...
<div class="row mb-5 text-center">
    <div class="col-lg-8 mx-auto">
        ...
        <p class="text-muted fs-5">Flexible pricing that scales with your business needs. Upgrade your service limits instantly.</p>
    </div>
</div>
...
<div class="row g-4 mb-5">
    <div class="col-md-4">
        <div class="card border-0 shadow-sm rounded-4 h-100 overflow-hidden text-start">
            <div class="card-body p-4 d-flex align-items-center">
                ...
                <div>
                    <small class="text-muted d-block fw-bold text-uppercase mb-1" style="font-size: 0.75rem; letter-spacing: 0.5px;">Active Limit</small>
                    <div class="d-flex align-items-baseline">
                        <h3 class="mb-0 fw-bold text-dark">{{ $businessSettings->service_limit }}</h3>
                        <span class="ms-2 text-muted small">Services</span>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="col-md-4">
        <div class="card border-0 shadow-sm rounded-4 h-100 overflow-hidden text-start">
            <div class="card-body p-4 d-flex align-items-center">
                @php
                $isActive = $businessSettings->service_limit_expiry_date && \Carbon\Carbon::parse($businessSettings->service_limit_expiry_date)->isFuture();
                @endphp
                <div class="bg-{{ $isActive ? 'success' : 'warning' }} bg-opacity-10 text-{{ $isActive ? 'success' : 'warning' }} rounded-4 p-3 me-3">
                    <i class="bi bi-shield-check fs-3"></i>
                </div>
                <div>
                    <small class="text-muted d-block fw-bold text-uppercase mb-1" style="font-size: 0.75rem; letter-spacing: 0.5px;">Plan Status</small>
                    <h4 class="mb-0 fw-bold text-{{ $isActive ? 'success' : 'warning' }}">
    ...
    <div class="col-md-4">
        <div class="card border-0 shadow-sm rounded-4 h-100 overflow-hidden text-start">
            <div class="card-body p-4 d-flex align-items-center">
                ...
                <div>
                    <small class="text-muted d-block fw-bold text-uppercase mb-1" style="font-size: 0.75rem; letter-spacing: 0.5px;">Validity Until</small>
                    <h4 class="mb-0 fw-bold text-dark">
                        {{ $businessSettings->service_limit_expiry_date ? get_date($businessSettings->service_limit_expiry_date) : 'N/A' }}
                    </h4>
                </div>
            </div>
        </div>
        ...
        <div class="card h-100 border-0 shadow-sm rounded-4 position-relative overflow-hidden plan-card">
            ...
            <div class="card-body p-4 text-center d-flex flex-column">
                ...
                <div class="p-3 bg-light rounded-3 mb-4">
                    <span class="d-block fw-bold fs-4 text-dark">{{ $site_setting->free_service_limit }}</span>
                    <span class="text-muted small text-uppercase fw-bold">Services Limit</span>
                </div>
    ...
    <div class="col-md-6 col-lg-4">
        <div class="card h-100 border-0 shadow rounded-4 position-relative overflow-hidden plan-card border-primary-hover">
            ...
            <div class="card-body p-4 text-center d-flex flex-column">
                ...
                <!-- Quantity Input -->
                <div class="mb-4">
                    <label class="form-label small text-uppercase text-muted fw-bold letter-spacing-1">Service Quantity</label>
                    <div class="input-group input-group-lg shadow-sm rounded-pill overflow-hidden border">
                        <input type="number" class="form-control text-center fw-bold border-0 plan-qty-input"
                            data-id="{{ $plan->id }}"
                            data-price="{{ $plan->per_service_price }}"
                            data-duration="{{ $plan->duration }}"
                            placeholder="25" min="1" max="{{ $plan->max_service_limit }}" value="25">
                    ...
                    <div class="d-flex justify-content-between mt-2 px-2">
                        ...
                        <small class="text-muted">Max: {{ $plan->max_service_limit ?? 250 }}</small>
                    </div>
                </div>
                ...
                <a href="#" class="btn btn-primary w-100 py-3 rounded-pill fw-bold shadow-sm mt-4 hover-scale buy-details-btn"
                    data-id="{{ $plan->id }}">
...
<script>
    $(function() {
        ...
        $('.buy-details-btn').click(function(e) {
            e.preventDefault();
            ...
            if (qty > maxLimit) {
                Swal.fire('Error', 'Maximum service limit is ' + maxLimit, 'error');
                return;
            }

            var url = "{{ route('business.service.plan.details', ':id') }}";
            url = url.replace(':id', planId) + "?quantity=" + qty;
