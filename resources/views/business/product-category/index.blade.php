...
<script type="text/javascript">
    ...
    $(function() {
        table = $('#data-table').DataTable({
            ...
            serverSide: true,
            ajax: {
                url: "{{ route('business.product-category.index') }}",
            },
            lengthChange: false,
            pageLength: 15,
            columnDefs: [{
    ...
    function editCategory(id) {
        ...
        $.get(url, function(response) {
            if (response.success) {
                $('#categoryModalLabel').text('Edit Product Category');
                $('#categoryForm').attr('action', "{{ route('business.product-category.update', ':id') }}".replace(':id', id));
    ...
    function openReorderModal() {
        $.get("{{ route('business.product-category.index') }}", {
            all: true
        }, function(response) {
            if (response.success) {
                let html = '';
    ...
    function saveOrder() {
        ...
        $.ajax({
            url: "{{ route('business.product-category.reorder') }}",
            type: "POST",
            data: {
                order: order,
                _token: "{{ csrf_token() }}"
