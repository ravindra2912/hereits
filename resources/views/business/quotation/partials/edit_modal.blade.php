<form id="editQuotationForm" action="{{ route('business.quotation.update', $quotation->id) }}" method="POST">
    ...
    <div class="modal-header border-0 pb-0 pt-4 px-4 sticky-top bg-white" style="z-index: 10;">
        <div class="d-flex justify-content-between w-100 align-items-center">
            <div>
                <h5 class="modal-title fw-bold"><i class="bi bi-pencil-square me-2 text-primary"></i>Edit Quotation #{{ $quotation->quotation_no }}</h5>
            </div>
                ...
                <div class="card border-0 shadow-sm bg-light rounded-4 mb-0">
                    ...
                    <div class="card-body p-4 pt-0">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="customer_name" id="modal_customer_name" class="form-control rounded-pill px-3" placeholder="Enter customer name" value="{{ $quotation->customer_name }}">
                        </div>

                        <div class="mb-3">
                            <label class="form-label fw-bold small text-secondary">Customer Contact</label>
                            <input type="text" name="customer_contact" id="modal_customer_contact" class="form-control rounded-pill px-3" placeholder="Enter customer contact" value="{{ $quotation->customer_contact }}">
                        </div>

                        <div class="mb-3" id="modal_customer_email_group" style="{{ $selectedCustomer ? 'display: none;' : '' }}">
                            <label class="form-label fw-bold small text-secondary">Customer Email <span class="text-danger">*</span></label>
                            <input type="email" name="customer_email" id="modal_customer_email" class="form-control rounded-pill px-3" placeholder="Enter customer email" value="{{ $selectedCustomer->email ?? '' }}">
                        </div>

                        <div class="mb-3">
                            <label class="form-label fw-bold small text-secondary">Address</label>
                            <textarea name="address" id="modal_address" class="form-control rounded-4" rows="2" placeholder="Address">{{ $quotation->address }}</textarea>
                        </div>

                        <div class="row mb-3">
                            <div class="col-6">
                                <label class="form-label fw-bold small text-secondary">City</label>
                                <input type="text" name="city" id="modal_city" class="form-control rounded-pill px-3" placeholder="City" value="{{ $quotation->city }}">
                            </div>
                            <div class="col-6">
                                <label class="form-label fw-bold small text-secondary">State</label>
                                <input type="text" name="state" id="modal_state" class="form-control rounded-pill px-3" placeholder="State" value="{{ $quotation->state }}">
                            </div>
                        </div>

                        <div class="mb-3">
                            <label class="form-label fw-bold small text-secondary">Pincode</label>
                            <input type="text" name="pincode" id="modal_pincode" class="form-control rounded-pill px-3" placeholder="Pincode" value="{{ $quotation->pincode }}">
                        </div>
                                ...
                                <table class="table table-sm table-borderless text-end align-middle mb-0">
                                    ...
                                    <tr>
                                        ...
                                        <td class="px-3" style="width: 150px;">
                                            <input type="number" name="discount" id="modal_summary_discount" class="form-control form-control-sm text-end rounded-pill px-3 py-1" value="{{ $quotation->discount }}" min="0" step="0.01">
                                        </td>
                                    </tr>
                                    <tr>
                                        <td class="text-secondary align-middle">Shipping Charge (₹):</td>
                                        <td class="px-3">
                                            <input type="number" name="shipping_charge" id="modal_summary_shipping" class="form-control form-control-sm text-end rounded-pill px-3 py-1" value="{{ $quotation->shipping_charge }}" min="0" step="0.01">
                                        </td>
                                    </tr>
                                    <tr>
                                        <td class="text-secondary align-middle">Tax (₹):</td>
                                        <td class="px-3">
                                            <input type="number" name="tax" id="modal_summary_tax" class="form-control form-control-sm text-end rounded-pill px-3 py-1" value="{{ $quotation->tax }}" min="0" step="0.01">
                                        </td>
                                    </tr>
            ...
            $.ajax({
                ...
                error: function(xhr) {
                    ...
                    if (xhr.status === 422) {
                        ...
                        toastr.error(xhr.responseJSON?.message || "Failed to update quotation.");
                    }
                }
            });
        });
